...
from src.f04_character_prompt import generate_character_prompts
...
def main():
    print("=" * 60)
    print("F04 测试：人物提示词撰写（批量模式）")
    print("=" * 60)

    # 用法: python tests/test_f04.py [f04_input.json]
    # 默认读取 tests/fixtures/f04_input.json
    input_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(FIXTURES_DIR, "f04_input.json")

    with open(input_path, "r", encoding="utf-8") as f:
        test_data = json.load(f)

    # characters 可以是直接数组，也可以是包含 list 属性的对象（兼容 f02 输出格式）
    characters_data = test_data.get("characters", [])
    ...
    result = generate_character_prompts(test_data, visual_tone)
    elapsed = time.time() - start_time

    outpath = os.path.join(OUTPUT_DIR, "f04_output.json")
    ...
    for r in result.get("results", []):
        ...
        print(f"      {status} {cid}: {len(prompt)} 字")
