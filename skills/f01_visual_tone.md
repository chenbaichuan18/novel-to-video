...
| `visual_style.reference_works` | **【强制一致性】**必须与 `medium` 类型匹配：<br>- `cinematic` → **必须**是真人电影/写实摄影作品，**禁止**动画作品<br>- `anime` → **必须**是动画作品，**禁止**真人电影<br>- `tv_series` → 电视剧/写实作品<br>- `documentary` → 纪录片风格作品<br>若文本风格类似某影视作品，列出1-2个；否则留空数组 |
...
### ⚠️ 【致命红线】reference_works 与 medium 一致性检查

> **【强制】`reference_works` 必须与 `medium` 类型严格匹配，禁止混用！**
>
> - 若 `medium=cinematic`（真人电影），`reference_works` **禁止**是动画作品（如《你的名字》《天气之子》《千与千寻》等宫崎骏作品）
> - 若 `medium=anime`（动画），`reference_works` **必须**是动画作品
> - 示例：`medium=cinematic` 时 → `reference_works=["《花样年华》", "《重庆森林》"]` ✅
> - 示例：`medium=cinematic` 时 → `reference_works=["《你的名字》"]` ❌（动画≠真人电影）
