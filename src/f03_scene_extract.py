...
if __name__ == "__main__":
    ...
    # 默认读 fixtures 输入文件
    default_input = _PROJECT_ROOT / "tests" / "fixtures" / "f03_input.json"
    input_path = _sys.argv[1] if len(_sys.argv) > 1 else str(default_input)

    with open(input_path, encoding="utf-8") as f:
        test_data = json.load(f)

    result = extract_scenes(test_data["text"], task_id=test_data.get("task_id"))

    # 写入 output 目录
    out_dir = _PROJECT_ROOT / "tests" / "output"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "f03_output.json"
