...
_ANIME_REFERENCE_KEYWORDS = (
    "你的名字", "天气之子", "铃芽", "千与千寻", "龙猫", "宫崎骏", "新海诚",
    "秒速五厘米", "言叶之庭", "哈尔的移动城堡", "幽灵公主", "风之谷",
)

_LIVE_ACTION_REFERENCE_KEYWORDS = (
    "花样年华", "重庆森林", "深夜食堂", "迷失东京", "霸王别姬", "色，戒",
    "地久天长", "山河故人", "小偷家族", "海街日记",
)
...
def _normalize_visual_tone(result: dict) -> dict:
    """执行代码层兜底，保证 F01 关键约束不会把下游带偏。"""
    visual_style = result.setdefault("visual_style", {})
    medium = _resolve_medium(str(visual_style.get("medium", ""))) or "cinematic"
    visual_style["medium"] = medium

    refs = visual_style.get("reference_works", [])
    ...
    for ref in refs:
        if not isinstance(ref, str) or not ref.strip():
            continue
        is_anime_ref = any(keyword in ref for keyword in _ANIME_REFERENCE_KEYWORDS)
        is_live_ref = any(keyword in ref for keyword in _LIVE_ACTION_REFERENCE_KEYWORDS)
        if medium in ("cinematic", "tv_series", "documentary") and is_anime_ref:
            logger.warning("F01 过滤与 medium=%s 不匹配的动画参考作品: %s", medium, ref)
            continue
        if medium == "anime" and is_live_ref:
            logger.warning("F01 过滤与 medium=anime 不匹配的真人参考作品: %s", ref)
            continue
        filtered_refs.append(ref)
    visual_style["reference_works"] = filtered_refs

    color_palette = result.setdefault("color_palette", {})
    dominant_colors = color_palette.get("dominant_colors", [])
    if isinstance(dominant_colors, list):
        color_palette["dominant_colors"] = [
            c.lower() if isinstance(c, str) and c.startswith("#") else c
            for c in dominant_colors
        ]
    accent_color = color_palette.get("accent_color")
    if isinstance(accent_color, str) and accent_color.startswith("#"):
        color_palette["accent_color"] = accent_color.lower()
    ...
    required_paths = [
        ...
        ("era_setting", "era"),
        ("color_palette", "dominant_colors"),
        ("atmosphere", "overall_mood"),
        ("atmosphere", "pacing_rhythm"),
    ]
    for parent, child in required_paths:
        value = result.get(parent, {}).get(child) if isinstance(result.get(parent), dict) else None
        if value in (None, "", []):
            raise ValueError(f"F01 输出缺少必填字段: {parent}.{child}")

    if not result.get("lighting_philosophy"):
        raise ValueError("F01 输出缺少必填字段: lighting_philosophy")
    if not result.get("visual_constraints"):
        raise ValueError("F01 输出缺少必填字段: visual_constraints")
    if not result.get("mood_keywords"):
        raise ValueError("F01 输出缺少必填字段: mood_keywords")
...
if __name__ == "__main__":
    ...
    print(f"视觉风格: {result.get('visual_style', {}).get('medium', 'N/A')}")
