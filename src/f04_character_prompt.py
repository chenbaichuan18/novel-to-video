...
def generate_character_prompt(character: dict, f01_data: dict, task_id: str = None) -> dict:
    """
    为单个角色生成定妆照提示词。
    ...
    Returns:
        {task_id, character_id, appearance, visual_style}
    """
    if task_id is None:
...
def generate_character_prompts(f02_output: dict, f01_data: dict, max_workers: int = 5) -> dict:
    """
    为 F02 输出的所有角色批量生成定妆照提示词（主入口，并发版本）。

    输入 F02 的完整输出（含 characters.list）和 F01 的完整输出，
    ...
    def _process_one(args: tuple) -> tuple:
        ...
        try:
            single_result = generate_character_prompt(char, f01_data)
...
if __name__ == "__main__":
    ...
    if result.get("results"):
        ...
        for idx, res in enumerate(result["results"], 1):
            ...
            if "error" not in res:
                appearance_text = res.get("appearance", "")
                # appearance 截取前 80 个字符用于显示
                display_text = appearance_text[:80] if len(appearance_text) > 80 else appearance_text
                print(f"  [{idx}] {char_id} {status} - 外貌描述: {display_text}...")
            else:
