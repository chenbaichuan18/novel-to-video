...
def generate_scene_prompt(scene: dict, f01_data: dict, task_id: str = None) -> dict:
    """
    为单个场景生成定妆照提示词。

    Args:
        scene: F03 输出的单个场景对象 (scenes.list[i])
        f01_data: F01 完整输出 JSON（含 visual_style 等字段）
        task_id: 任务 ID（可选，自动生成）

    Returns:
        {task_id, scene_id, scene_description, visual_style}
    """
    if task_id is None:
...
def generate_scene_prompts(f03_output: dict, f01_data: dict, max_workers: int = 5) -> dict:
    """
    为 F03 输出的所有场景批量生成定妆照提示词（主入口，并发版本）。

    输入 F03 的完整输出（含 scenes.list）和 F01 的完整输出，
    ...
    def _process_one(args: tuple) -> tuple:
        ...
        try:
            single_result = generate_scene_prompt(scene, f01_data)
...
if __name__ == "__main__":
    ...
    print(f"\n视觉基调: {visual_style.get('medium', '未知')}")
    ...
    if result.get("results"):
        ...
        for idx, res in enumerate(result["results"], 1):
            ...
            if "error" not in res:
                scene_desc = res.get("scene_description", "")
                vis_style = res.get("visual_style", "")
                # scene_description 截取前 80 个字符用于显示
                display_desc = scene_desc[:80] if len(scene_desc) > 80 else scene_desc
                print(f"  [{idx}] {scene_id} {status} - 场景描述: {display_desc}...")
            else:
